...
function sanitizeForStorage(value, fallback = '') {
...
async function ensureUserAssignedToAdmin(userId, email) {
    try {
...
async function handleFirebaseAuth(req, res) {
    ...
    try {
        initializeFirebase();
        decoded = await getFirebaseAuth().verifyIdToken(parsed.data.idToken);
    }
    catch (err) {
...
async function recordLoginMetadata(userId, ip, userAgent) {
    try {
        const current = await getUserById(userId);
        let prefs = {};
        try {
            if (current?.prefs)
                prefs = JSON.parse(current.prefs);
        }
        catch {
            prefs = {};
        }
        ...
        const history = Array.isArray(security.loginHistory) ? security.loginHistory : [];
        const entry = {
...
async function getSignupBonusConfig() {
...
async function awardSignupBonus(userId) {
    ...
    if (!config?.enabled || config.amountUsd <= 0)
        return;
...
async function ensureAdminTreasury(userId) {
...
export async function autoPromoteIfAdminEmail(userId, email, currentRole) {
...
function ensureDbReady(_req, _res, next) {
    next();
}
router.post('/signup', ensureDbReady, authLimiter, async (req, res) => {
    ...
    try {
        ...
        if (!otpResult.error && otpResult.code) {
            ...
            const pendingToken = signToken({ sub: user.id, email: user.email, v: user.tokenVersion ?? 0, otpPending: true, signupVerification: true });
...
router.post('/login/resend-otp', authLimiter, async (req, res) => {
    ...
    const pendingToken = signToken({ sub: user.id, email: user.email, v: user.tokenVersion ?? 0, otpPending: true });
...
router.post('/login', ensureDbReady, authLimiter, async (req, res) => {
    try {
        ...
        try {
            ...
            const errorCode = dbError && typeof dbError === 'object' && 'code' in dbError ? String(dbError.code ?? '') : '';
        ...
        if (!isTestRunner)
            otpRequired = user.role !== 'admin' || await shouldRequireOTPForLogin(user.id);
        if (otpRequired) {
...
router.post('/login/verify-otp', authLimiter, async (req, res) => {
    ...
    let payload = null;
    payload = verifyToken(parsed.data.pendingToken);
    if (!payload?.sub || !payload.otpPending) {
        res.status(401).json({ error: 'Invalid or expired session' });
        return;
    }
...
export async function promoteAllAdminEmails() {
    ...
    for (const email of adminEmails) {
        try {
            ...
            console.error(`[verdexis-api] failed to promote ${email}:`, e.message);
        }
    }
}
...
router.post('/change-password', requireAuth, requireCsrf, authLimiter, async (req, res) => {
