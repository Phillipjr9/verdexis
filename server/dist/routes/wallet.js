...
router.get('/', requireAuth, async (req, res) => {
    ...
    const allTransactions = [...transactions, ...pendingTxs].sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
    res.json({ balances: sanitizeBigInt(balances), transactions: sanitizeBigInt(allTransactions) });
});
...
router.get('/me/deposit-addresses', requireAuth, async (req, res) => {
    ...
    const addresses = prefs.depositAddresses ?? null;
    res.json({ addresses });
});
router.get('/saved-wallet', requireAuth, async (req, res) => {
    ...
    const savedWallet = prefs.savedWallet;
...
router.post('/saved-wallet', requireAuth, async (req, res) => {
...
router.delete('/saved-wallet', requireAuth, async (req, res) => {
    const user = await prisma.user.findUnique({ where: { id: req.userId }, select: { prefs: true } });
    let prefs = {};
    try {
        if (user?.prefs)
...
router.post('/transactions', requireAuth, moneyLimiter, idempotency(), async (req, res) => {
    ...
    const result = await prisma.$transaction(async (tx) => {
        if (userDepositRequiresApproval) {
            ...
            return { transaction, pendingApproval: true };
        }
...
router.post('/convert', requireAuth, moneyLimiter, idempotency(), async (req, res) => {
    ...
    if (fromCurrency === toCurrency) {
        res.status(400).json({ error: 'Cannot convert to the same currency' });
        return;
    }
    ...
    }).catch((err) => ({ error: err.message, status: err.status || 500 }));
    if ('error' in result) {
        res.status(result.status || 500).json({ error: result.error });
        return;
    }
...
router.post('/swap', requireAuth, moneyLimiter, idempotency(), async (req, res) => {
    ...
    if (fromCurrency === toCurrency) {
        res.status(400).json({ error: 'Cannot swap to same currency' });
        return;
    }
    if (req.userRole !== 'admin') {
        const u = await prisma.user.findUnique({
    ...
    const result = await prisma.$transaction(async (tx) => {
        const src = await tx.walletBalance.findUnique({ where: { userId_currency: { userId: req.userId, currency: fromCurrency } } });
        if (!src || src.available < amount) {
            throw Object.assign(new Error(`Insufficient ${fromCurrency}`), { status: 400 });
        }
        const rates = { USD: 1, USDC: 1, USDT: 1, BTC: 67432, ETH: 3521, SOL: 178.45, ADA: 0.52, XRP: 0.55, DOGE: 0.12, MATIC: 0.62, DOT: 6.8, AVAX: 32, LINK: 14, LTC: 75, BCH: 380 };
...
router.post('/transfer', requireAuth, moneyLimiter, idempotency(), async (req, res) => {
    ...
    if (sender?.prefs && sender.role !== 'admin') {
        try {
            const prefs = JSON.parse(sender.prefs);
            if (prefs.bonusLocked === true) {
                res.status(423).json({
    ...
    if (sender?.dailyTransferLimit || sender?.monthlyTransferLimit) {
        ...
        const monthSum = recent.reduce((s, t) => s + Math.abs(t.amount), 0);
        const daySum = recent.filter((t) => t.createdAt >= dayAgo).reduce((s, t) => s + Math.abs(t.amount), 0);
    ...
    const result = await prisma.$transaction(async (tx) => {
        const senderBal = await tx.walletBalance.findUnique({ where: { userId_currency: { userId: req.userId, currency } } });
...
async function mirrorPrimaryToUser(tx, userId, primary) {
...
router.post('/links', requireAuth, async (req, res) => {
    ...
    const link = await prisma.$transaction(async (tx) => {
        const existing = await tx.walletLink.findUnique({ where: { userId_address: { userId: req.userId, address } } });
        const otherCount = await tx.walletLink.count({ where: { userId: req.userId, NOT: { address } } });
        ...
        if (wantsPrimary) {
            ...
            await tx.walletLink.updateMany({ where: { userId: req.userId, isPrimary: true, NOT: { address } }, data: { isPrimary: false } });
        }
...
router.delete('/links/:id', requireAuth, async (req, res) => {
    ...
    const result = await prisma.$transaction(async (tx) => {
        ...
        if (row.isPrimary) {
            ...
            const next = await tx.walletLink.findFirst({ where: { userId: req.userId }, orderBy: { linkedAt: 'desc' } });
            if (next) {
...
router.post('/links/:id/primary', requireAuth, async (req, res) => {
    ...
    const result = await prisma.$transaction(async (tx) => {
        ...
        if (!row)
            return { ok: false };
        await tx.walletLink.updateMany({ where: { userId: req.userId, isPrimary: true, NOT: { id } }, data: { isPrimary: false } });
...
router.post('/pending-deposits', requireAuth, moneyLimiter, idempotency(), async (req, res) => {
...
router.get('/pending-deposits', requireAuth, async (req, res) => {
...
router.put('/deposit-instructions', requireAuth, requireAdmin, async (req, res) => {
    const body = req.body;
