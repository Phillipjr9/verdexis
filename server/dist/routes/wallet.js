...
router.get('/', requireAuth, async (req, res) => {
    ...
    let transactions = [];
    try {
        const rows = await withTimeout(prisma.walletBalance.findMany({ where: { userId }, orderBy: { currency: 'asc' } }), 8_000, 'walletBalance');
    ...
    try {
        ...
        for (const h of holdings) {
            ...
            if (existing && Math.abs(existing.available - amt) < 1e-9 && Math.abs(existing.balance - amt) < 1e-9)
                continue;
    ...
    try {
        transactions = await withTimeout(prisma.transaction.findMany({
            where: { userId },
...
async function lookupRecipientHandler(req, res) {
    const raw = (req.body && req.body.email) ?? req.query?.email;
...
router.post('/transfer', requireAuth, idempotency(), async (req, res) => {
            ...
            if (fromCurrency !== 'USD') {
                ...
                if (h) {
                    ...
                    if (nextAmt <= 0)
                        ...
                        await tx.holding.update({ where: { id: h.id }, data: { amount: nextAmt } });
                }
            }
            const { generateTransactionId } = await import('../utils/transactionIdGenerator.js');
