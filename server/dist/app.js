...
const normalizeOrigin = (value) => value.trim().toLowerCase()
    .replace(/\/+$/g, '')
    .replace(/\s+/g, '')
    .replace(/:443$/, '')
    .replace(/:80$/, '');
...
app.use(cors({
    origin: (origin, cb) => {
        ...
        if (ALLOWED_ORIGINS.has(normalizedOrigin) ||
            ...
            return cb(null, true);
        if (!IS_PROD && LAN_ORIGIN_RE.test(normalizedOrigin))
            return cb(null, true);
...
// Global response sanitizer: convert BigInt -> string before JSON serialization
app.use((req, res, next) => {
    const originalJson = res.json.bind(res);
    res.json = (body) => {
        try {
            return originalJson(sanitizeBigInt(body));
        }
        catch (e) {
