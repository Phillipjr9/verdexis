...
import { emailLinks, emailLogoUrl, prepareCustomerEmailHtml } from '../config/email.js'
...
class EmailService {
  ...
  async send(options: EmailOptions): Promise<boolean> {
    try {
      ...
      const html = prepareCustomerEmailHtml(options.html)
      const success = await sendEmailNotification(options.to, options.subject, plainText, html, {
  ...
  async sendOTP(to: string, userName: string, otp: string, expirationMinutes: number, userId?: string): Promise<boolean> {
    ...
    const brandedHtml = prepareCustomerEmailHtml(html)
    ...
    const direct = await this.sendOtpDirect(to, subject, text, brandedHtml)
