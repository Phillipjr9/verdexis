...
router.post('/signup', authLimiter, async (req, res) => {
  try {
    ...
    // Referral program: assign personal code + link referrer if program enabled
    try {
      await ensureUserReferralCode(user.id)
      if (referralToken) {
        await linkReferrer(user.id, normalizedEmail, referralToken)
      }
    } catch (refErr) {
      console.warn('[auth] referral link skipped', refErr instanceof Error ? refErr.message : refErr)
    }
