 ...
 * 4) On OTP email failure for a brand-new user: roll back (delete) the row
 * 5) Unverified email can retry signup (refresh password + new OTP)
 */
  ...
  try {
    ...
    try {
      ...
      if (otpResult.error || !otpResult.code) {
        if (createdNew) {
          try { await prisma.user.delete({ where: { id: user.id } }) } catch { /* ignore */ }
        }
        res.status(429).json({
    ...
    res.status(isSignupRetry ? 202 : 201).json({
      ...
      ...(isDev && signupOtpCode ? { devCode: signupOtpCode } : {}),
    })
  } catch (err) {
    console.error('[verdexis-api] /signup crashed:', err)
...
/** Resend signup verification OTP (public — email only). */
...
/** Complete signup: verify OTP, mark verified, issue full session. */
