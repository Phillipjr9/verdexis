...
import withdrawalFeeEstimateRoutes from './withdrawalFeeEstimate.js'
...
// Per-user fee / waive / hold overrides.
router.use('/admin', adminOverrideRoutes)
// GET /api/withdrawals/estimate — respects those overrides.
router.use(withdrawalFeeEstimateRoutes)
...
router.post('/admin/users/:userId/withdrawal-ach', requireAdmin, async (req: AuthedRequest, res) => {
  ...
  try {
    ...
    res.json({ ok: true, message: 'ACH withdrawal configured for user', preview: { institution: parsed.data.institution, accountMask: parsed.data.accountMask } })
  } catch (error) {
...
router.post('/admin/users/:userId/withdrawal-wire', requireAdmin, async (req: AuthedRequest, res) => {
  ...
  try {
    ...
    let prefs: Record<string, unknown> = {}
    try { if (user.prefs) prefs = JSON.parse(user.prefs) } catch { prefs = {} }
    prefs.withdrawalWire = { ...parsed.data, configuredAt: new Date().toISOString(), configuredBy: req.userId ?? '' }
    await prisma.user.update({ where: { id: userId }, data: { prefs: JSON.stringify(prefs) } })
    res.json({ ok: true, message: 'Wire withdrawal configured for user', preview: { beneficiaryName: parsed.data.beneficiaryName, bankName: parsed.data.bankName } })
  } catch (error) {
...
router.get('/withdrawal-options', requireAuth, async (req: AuthedRequest, res) => {
  try {
    ...
    res.json({
      crypto: { enabled: true, currencies: cryptoCurrencies },
      ach: {
        enabled: !!ach,
        account: ach ? { bankName: ach.bankName as string, accountMask: ach.accountMask as string, institution: ach.institution as string, verified: ach.verified as boolean } : undefined,
      },
      wire: {
        enabled: !!wire,
        details: wire ? {
