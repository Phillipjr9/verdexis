...
import { notifyWithdrawalEvent } from '../services/emailHooks.js'
...
router.post('/', requireAuth, moneyLimiter, idempotency(), async (req: AuthedRequest, res) => {
  ...
  try {
    ...
    const prepared = await prisma.$transaction(async (tx) => {
      ...
      const normalizedAssetInner = asset.toUpperCase()
      ...
      const withdrawal = await tx.withdrawalRequest.create({ data: { userId: req.userId!, walletLinkId: walletLink.id, amount, asset: normalizedAssetInner, fee: processingFee, status: 'pending' } })
    ...
    process.nextTick(() => {
      prisma.user.findUnique({ where: { id: req.userId! }, select: { id: true, email: true, name: true } }).then((u) => {
        ...
        notifyWithdrawalEvent(u, {
          ...
          txHash: (transfer as any).txHash ?? null,
          fee: processingFee,
          id: finalized.id,
        }).catch(() => {})
      }).catch(() => {})
