...
export type AdminEmailOptions = {
  ...
  important?: boolean
  /** Optional subset of the configured admin allowlist. Non-allowlisted addresses are dropped. */
  recipients?: string[]
}

const EMAIL_RE = /^[^\s@]+@[^\s@]+\.[^\s@]+$/

/** Recipients may only be members of ADMIN_EMAIL / ADMIN_EMAILS (allowlist). */
function resolveAdminRecipients(requested?: string[]): string[] {
  ...
  const source = (requested?.length ? requested : Array.from(allow))
    .map((a) => a.trim().toLowerCase())
    .filter(Boolean)

  return Array.from(new Set(source.filter((a) => allow.has(a) && EMAIL_RE.test(a))))
}

/**
 * Secure internal admin mail path:
 * - From display: Verdexis Admin <admin@verdexisgroup.com>
 * - Envelope MAIL FROM: SMTP auth user when set (SPF/DMARC alignment)
 * - Recipients restricted to ADMIN_EMAIL / ADMIN_EMAILS allowlist only
 * - No customer tracking / List-Unsubscribe headers
 * - Optional high-importance headers
 */
export async function sendAdminEmailNotification(
  ...
  try {
    const transporter = getEmailTransporter()
    if (!transporter) {
      console.warn('[notification-service] Admin email skipped: SMTP not configured')
      return false
    }
    const config = resolveEmailTransportConfig()
    ...
    if (!recipients.length) {
      console.warn('[notification-service] Admin email blocked: no allowlisted recipients')
      return false
    }
    ...
    const headers: Record<string, string> = {
      'X-Mailer': 'Verdexis',
      'X-Verdexis-Channel': 'admin-internal',
      'Auto-Submitted': 'auto-generated',
      'Precedence': 'bulk',
      'Sender': formatEmailAddress(envelopeFrom, 'Verdexis Admin'),
    }
    ...
    const safeHtml =
      htmlBody ??
      `<p style="font-family:Segoe UI,Arial,sans-serif;line-height:1.5">${escapeHtml(body).replace(/\n/g, '<br />')}</p>`

    console.log('[notification-service] Sending admin email:', {
      toCount: recipients.length,
      subject: safeSubject.slice(0, 80),
