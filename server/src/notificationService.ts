...
export async function sendWithdrawalNotification(userId: string, amount: number, asset: string, status: 'pending' | 'approved' | 'rejected' | 'completed'): Promise<void> {
  ...
  const messages: Record<string, { subject: string; body: string }> = {
    ...
    rejected: { subject: 'Withdrawal Rejected', body: `Your withdrawal request for ${amount} ${asset} has been rejected. Funds have been returned to your balance.` },
  ...
  // Money events always email the user (also creates in-app notification)
  await sendEmailNotification(user.email, message.subject, message.body, undefined, { userId, kind: 'transaction', title: message.subject, body: message.body })
}

export async function sendDepositNotification(userId: string, amount: number, asset: string, status: 'pending' | 'confirmed' | 'credited' | 'rejected'): Promise<void> {
  const user = await prisma.user.findUnique({ where: { id: userId }, select: { email: true } }); if (!user) return
  const messages: Record<string, { subject: string; body: string }> = {
    ...
    rejected: { subject: 'Deposit Rejected', body: `Your deposit of ${amount} ${asset} was not approved. Please contact support if you have questions.` },
  }
  const message = messages[status]; if (!message) return
  // Money events always email the user (also creates in-app notification)
  await sendEmailNotification(user.email, message.subject, message.body, undefined, { userId, kind: 'transaction', title: message.subject, body: message.body })
