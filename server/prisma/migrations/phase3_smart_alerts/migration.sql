...
-- Add new columns to PriceAlert for multiple alert types
...
ALTER TABLE "PriceAlert" ADD COLUMN IF NOT EXISTS "percentageChange" FLOAT;

-- Add time window column (hours for percentage alerts)
ALTER TABLE "PriceAlert" ADD COLUMN IF NOT EXISTS "timeWindow" INT DEFAULT 24;

-- Add portfolio target column (for portfolio value alerts)
ALTER TABLE "PriceAlert" ADD COLUMN IF NOT EXISTS "portfolioTarget" FLOAT;

-- Create index for faster alert lookups
CREATE INDEX IF NOT EXISTS "idx_alert_type_active" ON "PriceAlert"("alertType", "active");
