...
import { RefreshCw, Search, Activity, Download } from 'lucide-react'
...
export default function AdminAudit() {
  ...
  return (
    <AdminLayout title="Audit log" subtitle="Platform actions and operator history">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3 mb-6">
        <div className="flex gap-2 items-center flex-wrap">
          ...
          <button type="button" onClick={downloadCsv} className="inline-flex items-center gap-2 px-3 py-2 bg-[#1a1a1a] border border-[#ffffff10] text-sm text-[#A0A0A0] rounded-lg hover:border-[#0C8B44]/40">
            <Download className="w-4 h-4" />Export CSV
          </button>
          <button type="button" onClick={load} className="inline-flex items-center gap-2 px-3 py-2 bg-[#0C8B44] text-white text-sm rounded-lg hover:bg-[#0a7539]">
      ...
      <div className="relative mb-4 max-w-md">
        ...
        <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Filter by action, actor, target or payload" className="w-full pl-9 pr-3 py-2 bg-[#0f1619] border border-[#ffffff10] rounded-lg text-sm text-[#E5E5E5] focus:outline-none focus:border-[#0C8B44]" />
      </div>
