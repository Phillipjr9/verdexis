...
export default function AdminDashboard() {
  ...
  const pagedTx = useMemo(() => {
    const start = (txSafePage - 1) * txPageSize
    return filteredTx.slice(start, start + txPageSize)
  }, [filteredTx, txSafePage, txPageSize])
  const txFrom = filteredTx.length === 0 ? 0 : (txSafePage - 1) * txPageSize + 1
  const txTo = Math.min(txSafePage * txPageSize, filteredTx.length)
  ...
  return (
    ...
    >
      ...
      <div className="mb-6 rounded-2xl bg-[#0f1619]/50 border border-[#ffffff08] overflow-hidden">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 px-4 py-3 border-t border-[#ffffff08]">
          <div className="flex items-center gap-2 text-xs text-[#737373]">
            <span>
              Showing {txFrom}–{txTo} of {filteredTx.length}
            </span>
            ...
            <label className="inline-flex items-center gap-1.5">
              ...
              <select
                ...
                className="rounded-lg bg-[#0a0f11] border border-[#ffffff12] text-[#E5E5E5] text-xs px-2 py-1 focus:outline-none focus:border-[#0C8B44]/50"
              >
                {[5, 10, 25, 50].map((n) => (
          ...
          <div className="flex items-center gap-1">
            <button
              ...
              className="inline-flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-xs border border-[#ffffff12] text-[#E5E5E5] disabled:opacity-40 disabled:cursor-not-allowed hover:bg-[#ffffff08]"
            >
              <ChevronLeft className="w-3.5 h-3.5" />
              Prev
            </button>
            <span className="px-3 text-xs text-[#A0A0A0]">
              Page {txSafePage} / {txTotalPages}
            </span>
            <button
              type="button"
