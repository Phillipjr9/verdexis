...
import { MegaphoneIcon, Send, AlertTriangle } from 'lucide-react'
...
export default function AdminBroadcast() {
  ...
  return (
    <AdminLayout title="Broadcast" subtitle="Send a message to every active user">
      <div className="grid lg:grid-cols-2 gap-6">
        <form onSubmit={send} className="rounded-2xl bg-[#0f1619]/50 border border-[#ffffff08] p-6 space-y-4">
          ...
          <label className="flex items-start gap-2 text-xs text-[#A0A0A0] cursor-pointer">
            ...
            <span>I understand this will create a notification for every active user. Recorded in the audit log.</span>
          </label>
        ...
        <div className="space-y-4">
          ...
          <div className="rounded-2xl bg-[#F57C00]/5 border border-[#F57C00]/30 p-4">
            ...
            <p className="text-[11px] text-[#A0A0A0]">Suspended accounts are skipped. No recall after send.</p>
          </div>

          {lastResult && (
            <div className="rounded-2xl bg-[#0C8B44]/5 border border-[#0C8B44]/30 p-4">
              <p className="text-xs text-[#0C8B44]">Last broadcast: {lastResult.count} users at {lastResult.at}</p>
            </div>
          )}
        </div>
      </div>
