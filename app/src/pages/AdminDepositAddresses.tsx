...
import { Save, QrCode, Trash2, Plus, Download, Building2, Banknote, FileText } from 'lucide-react'
...
const EMPTY_CHECK: CheckDetails = {
  payTo: '',
  addressLine1: '',
  addressLine2: '',
  city: '',
...
export default function AdminDepositAddresses() {
    ...
    try {
      ...
      if (res.addresses) {
        ...
        setAddresses({
          ...
          check: data.check ? { ...EMPTY_CHECK, ...data.check } : undefined,
          notes: data.notes,
        })
        if (data.cryptos) {
  ...
  const ensureCheck = () => {
    setAddresses(prev => ({
      ...prev,
      check: prev.check || { ...EMPTY_CHECK },
    }))
  }

  const updateCheck = (field: keyof CheckDetails, value: string | boolean) => {
    setAddresses(prev => ({
      ...prev,
      check: {
        ...(prev.check || EMPTY_CHECK),
        [field]: value,
      },
    }))
  }
  ...
  const clearCheck = () => {
    if (!confirm('Remove check deposit instructions for this user?')) return
    setAddresses(prev => {
      const next = { ...prev }
      delete next.check
      return next
  ...
  const saveAddresses = async () => {
    ...
    if (addresses.check) {
      const c = addresses.check
      const hasAny = c.payTo.trim() || c.addressLine1.trim() || c.city.trim()
      if (hasAny) {
        if (!c.payTo.trim() || !c.addressLine1.trim() || !c.city.trim() || !c.state.trim() || !c.postalCode.trim()) {
          toast.error('Check deposit: pay-to, street, city, state, and postal code are required')
          return
        }
      }
    }
    ...
    try {
      ...
      if (
        addresses.check &&
        addresses.check.payTo.trim() &&
        addresses.check.addressLine1.trim() &&
        addresses.check.city.trim() &&
        addresses.check.state.trim() &&
        addresses.check.postalCode.trim()
      ) {
        payload.check = {
          ...addresses.check,
          country: addresses.check.country?.trim() || 'United States',
          acceptPersonal: addresses.check.acceptPersonal !== false,
          acceptCashiers: addresses.check.acceptCashiers !== false,
        }
      }
      await api.updateUserDepositAddresses(userId, payload)
      toast.success('Deposit destinations saved (crypto + wire/ACH + check)')
      await loadAddresses()
    } catch (err) {
            ...
            <div className="flex items-center justify-between gap-4 flex-wrap">
              <div>
                ...
                <p className="text-xs text-[#0C8B44] mt-2 max-w-xl">
                  Set crypto addresses, bank (wire / ACH), and check mailing details this user should send funds to.
          ...
          <div className="glass-card p-6 mb-6">
            <div className="flex items-center justify-between mb-6">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-xl font-light text-[#E5E5E5]">Check deposit instructions</h2>
                  <p className="text-xs text-[#737373] mt-0.5">
                    Mailing address and payee name shown when the user chooses Check on the deposit tab.
                  </p>
                </div>
              </div>
              <div className="flex gap-2">
                ...
                {check && (
                  <button
                    ...
                    onClick={clearCheck}
                    className="p-2 text-[#FF5252] hover:bg-[#FF5252]/10 rounded-lg transition-colors"
                    title="Remove check deposit instructions"
                  >
                    <Trash2 className="w-4 h-4" />
                  </button>
                )}
            ...
            {!check ? (
              <div className="text-center py-10 text-[#737373]">
                ...
                <p>No check deposit instructions yet</p>
                <p className="text-sm mt-1">Add a pay-to name and mailing address so the user can mail a check or cashier's check.</p>
              </div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="md:col-span-2">
                  <Field label="Payable to *" value={check.payTo} onChange={(v) => updateCheck('payTo', v)} placeholder="Verdexis Holdings LLC" />
                </div>
                <div className="md:col-span-2">
                  <Field label="Address line 1 *" value={check.addressLine1} onChange={(v) => updateCheck('addressLine1', v)} placeholder="123 Market Street, Suite 400" />
                </div>
                <div className="md:col-span-2">
                  <Field label="Address line 2" value={check.addressLine2 || ''} onChange={(v) => updateCheck('addressLine2', v)} placeholder="Floor / unit (optional)" />
                </div>
                <Field label="City *" value={check.city} onChange={(v) => updateCheck('city', v)} placeholder="San Francisco" />
                <Field label="State / region *" value={check.state} onChange={(v) => updateCheck('state', v)} placeholder="CA" />
                <Field label="Postal code *" value={check.postalCode} onChange={(v) => updateCheck('postalCode', v)} placeholder="94105" />
                <Field label="Country *" value={check.country} onChange={(v) => updateCheck('country', v)} placeholder="United States" />
                <div className="md:col-span-2">
                  <Field label="Memo on check (user must write this)" value={check.memo || ''} onChange={(v) => updateCheck('memo', v)} placeholder="e.g. investment ID or email" />
                </div>
                <div className="md:col-span-2 grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <label className="flex items-center gap-3 p-3 rounded-lg border border-[#ffffff10] bg-[#0a0e10] cursor-pointer">
                    <input
                      type="checkbox"
                      checked={check.acceptPersonal !== false}
                      onChange={(e) => updateCheck('acceptPersonal', e.target.checked)}
                      className="rounded border-[#ffffff30]"
                    />
                    <div>
                      <p className="text-sm text-[#E5E5E5]">Accept personal checks</p>
                      <p className="text-xs text-[#737373]">Drawn on the user's bank account</p>
                    </div>
                  </label>
                  <label className="flex items-center gap-3 p-3 rounded-lg border border-[#ffffff10] bg-[#0a0e10] cursor-pointer">
                    <input
                      type="checkbox"
                      checked={check.acceptCashiers !== false}
                      onChange={(e) => updateCheck('acceptCashiers', e.target.checked)}
                      className="rounded border-[#ffffff30]"
                    />
                    <div>
                      <p className="text-sm text-[#E5E5E5]">Accept cashier's checks</p>
                      <p className="text-xs text-[#737373]">Bank-issued official checks</p>
                    </div>
                  </label>
                </div>
                <div className="md:col-span-2">
                  ...
                  <textarea
                    value={check.notes || ''}
                    onChange={(e) => updateCheck('notes', e.target.value)}
                    placeholder="e.g. Mail certified/return receipt. Allow 5–10 business days after delivery for credit."
                    rows={2}
                    className="w-full bg-[#070C0E] border border-[#ffffff15] rounded-lg px-4 py-2 text-[#E5E5E5] focus:outline-none focus:border-[#0C8B44]"
                  />
                </div>
