...
function ReferralInner() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const [summary, listData] = await Promise.all([api.getReferralSummary(), api.getReferralList()])
        ...
        if (summary.program) {
          setProgram({
            enabled: summary.program.enabled === true,
            referrerBonusUsd: Number(summary.program.referrerBonusUsd) || 0,
            refereeBonusUsd: Number(summary.program.refereeBonusUsd) || 0,
            minDepositUsd: Number(summary.program.minDepositUsd) || 0,
          })
        }
      } catch {
        ...
        <div className="max-w-4xl mx-auto">
          ...
          {!program.enabled && (
            <div className="mb-6 rounded-xl border border-yellow-500/30 bg-yellow-500/10 px-4 py-3 text-sm text-yellow-200">
              The referral program is currently paused by the admin. Your code is still available, but new invites will not earn bonuses until the program is turned back on.
            </div>
          )}
          ...
          <div className="rounded-2xl bg-gradient-to-br from-[#0C8B44]/20 to-[#0f1619] border border-[#0C8B44]/20 p-8 mb-8 text-center">
            ...
            <p className="text-sm text-[#737373] mb-6 max-w-md mx-auto">
              Invite friends to Verdexis. You get{' '}
              <span className="text-[#0C8B44] font-medium">${program.referrerBonusUsd}</span> in trading credits for every
              friend who signs up and makes a first deposit of at least ${program.minDepositUsd}.
            </p>
              ...
              <>
                <div className="flex items-center gap-2 max-w-lg mx-auto">
                  ...
                  <button
                    type="button"
                    onClick={copy}
                    className="flex items-center gap-2 px-4 py-3 bg-[#0C8B44] text-white text-xs font-medium rounded-xl hover:bg-[#0a7539] transition-colors shrink-0"
                  >
                ...
                <div className="flex items-center justify-center gap-4 mt-4">
                  <span className="text-xs text-[#737373]">
                    Your code: <span className="text-[#E5E5E5] font-mono">{referralCode}</span>
                  </span>
                  <button
                    type="button"
                    onClick={copy}
                    className="flex items-center gap-1 text-xs text-[#0C8B44] hover:text-[#0a7539] transition-colors"
                  >
            ...
            <div className="grid grid-cols-3 gap-6">
              {[
                { step: '01', title: 'Share your link', desc: 'Send your unique referral link or code to friends.' },
                {
                  ...
                  desc: `They create an account using your link and make a deposit ≥ $${program.minDepositUsd}.`,
                },
                {
                  step: '03',
                  title: 'Both get rewarded',
                  desc: `You receive $${program.referrerBonusUsd} in trading credits. Your friend gets a $${program.refereeBonusUsd} bonus.`,
                },
              ].map((s) => (
                <div key={s.step} className="text-center">
                  <div className="w-8 h-8 rounded-full bg-[#0C8B44]/15 text-[#0C8B44] text-xs font-bold flex items-center justify-center mx-auto mb-3">
                    {s.step}
                  </div>
            ...
            {loading ? (
              ...
              <div className="space-y-3">
                {referrals.map((r) => (
                  <div
                    key={r.id}
                    className="flex items-center justify-between rounded-xl bg-[#0a0f11] border border-[#ffffff08] px-4 py-3"
                  >
                    <div className="flex items-center gap-3">
                      <div className="w-7 h-7 rounded-full bg-[#0C8B44]/15 flex items-center justify-center text-[10px] font-bold text-[#0C8B44]">
                        {r.refereeEmail[0]?.toUpperCase()}
                      </div>
                      <div>
                        <p className="text-xs text-[#E5E5E5]">{r.refereeEmail}</p>
                        <p className="text-[10px] text-[#737373]">
                          {r.firstDepositAt
                            ? `Joined ${new Date(r.firstDepositAt).toLocaleDateString()}`
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <span className="text-xs text-[#E5E5E5]">
                        {r.referrerBonusUsd ? `+$${r.referrerBonusUsd}` : '—'}
                      </span>
                    </div>
                  </div>
                ))}
