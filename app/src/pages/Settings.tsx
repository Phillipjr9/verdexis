...
import {
  ...
  Fingerprint, Gift, Activity, Plug, AlertTriangle,
} from 'lucide-react'
import { listPasskeys, registerPasskey, deletePasskey, isPasskeySupported, type Passkey } from '../lib/passkeys'
...
function PasskeysPanel() {
  ...
  async function addKey() {
    if (!supported || !secure) {
      toast.error('Passkeys need a secure production site or localhost')
      return
    }
    const name = window.prompt('Device name', 'This device') || 'This device'
    setBusy(true)
    try {
  ...
  return (
    <div className="rounded-xl border border-[#ffffff08] bg-[#070C0E]/70 p-4 space-y-3">
      <div className="flex items-start gap-3">
        <Fingerprint className="w-5 h-5 text-[#0C8B44] mt-0.5" />
        <div className="flex-1 min-w-0">
          <p className="text-sm font-medium text-[#E5E5E5]">Passkeys</p>
          <p className="text-xs text-[#737373] mt-1">Sign in with fingerprint, Face ID, or a hardware key instead of a password.</p>
        </div>
        <button
          type="button"
          onClick={() => void addKey()}
          disabled={busy || !supported}
          className="shrink-0 rounded-lg bg-[#0C8B44] px-3 py-1.5 text-xs font-medium text-white disabled:opacity-40"
        >
          {busy ? 'Working…' : 'Add passkey'}
        </button>
      </div>
      {!secure && (
        <p className="text-xs text-[#F57C00]">Passkeys need HTTPS in production. Localhost is allowed for testing.</p>
      )}
      {loading ? (
        <p className="text-xs text-[#737373]">Loading passkeys…</p>
      ) : items.length === 0 ? (
        <p className="text-xs text-[#737373]">No passkeys on this account yet.</p>
      ) : (
        <ul className="space-y-2">
          {items.map((pk) => (
            <li key={pk.id} className="flex items-center justify-between gap-3 rounded-lg border border-[#ffffff08] px-3 py-2">
              ...
              <button type="button" onClick={() => void removeKey(pk.id)} className="text-xs text-[#f44336] hover:underline">
                Remove
              </button>
            </li>
          ))}
        ...
        <div className="max-w-3xl mx-auto space-y-6">
          ...
          <section className="rounded-2xl border border-[#ffffff08] bg-[#0f1619]/60 p-6 space-y-5">
            ...
            <div className="flex items-center gap-4">
              <button type="button" onClick={() => fileRef.current?.click()} className="relative h-16 w-16 overflow-hidden rounded-2xl border border-[#ffffff12] bg-[#1a1a1a] shrink-0" aria-label="Change profile photo">
                {avatar ? (
              ...
              <input ref={fileRef} type="file" accept="image/*" className="hidden" onChange={(e) => { void handleAvatar(e.target.files?.[0]); e.currentTarget.value = '' }} />
              <div>
          ...
          <section className="rounded-2xl border border-[#ffffff08] bg-[#0f1619]/60 p-6 space-y-4">
            ...
            <div className="flex flex-wrap items-center justify-between gap-3">
              ...
              <Link to="/kyc" className="rounded-lg bg-[#0C8B44] px-4 py-2 text-xs font-medium text-white hover:bg-[#0a7539]">Manage verification</Link>
            </div>
          </section>
            ...
            <div>
              ...
              <div className="grid grid-cols-3 gap-2">
                {(['dark', 'light', 'auto'] as ThemePref[]).map((t) => (
                  <button key={t} type="button" onClick={() => void persistPrefs({ ...prefs, theme: t })} className={`rounded-xl border px-3 py-2.5 text-sm capitalize ${prefs.theme === t ? 'border-[#0C8B44] bg-[#0C8B44]/10 text-[#E5E5E5]' : 'border-[#ffffff08] bg-[#1a1a1a]/50 text-[#A0A0A0]'}`}>{t}</button>
                ))}
              </div>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <label className="block">
                ...
                <select value={prefs.displayCurrency} onChange={(e) => void persistPrefs({ ...prefs, displayCurrency: e.target.value })} className="w-full rounded-xl border border-[#ffffff08] bg-[#1a1a1a] px-3 py-2.5 text-sm text-[#E5E5E5] focus:outline-none focus:border-[#0C8B44]">
                  {['USD', 'EUR', 'GBP', 'BTC', 'ETH'].map((c) => <option key={c} value={c}>{c}</option>)}
                </select>
              </label>
              <label className="block">
                <span className="text-xs text-[#A0A0A0] mb-1.5 block">Language</span>
                <select value={prefs.language} onChange={(e) => void persistPrefs({ ...prefs, language: e.target.value })} className="w-full rounded-xl border border-[#ffffff08] bg-[#1a1a1a] px-3 py-2.5 text-sm text-[#E5E5E5] focus:outline-none focus:border-[#0C8B44]">
              ...
              <label className="block sm:col-span-2">
                <span className="text-xs text-[#A0A0A0] mb-1.5 block">Timezone</span>
                <input value={prefs.timezone} onChange={(e) => void persistPrefs({ ...prefs, timezone: e.target.value })} className="w-full rounded-xl border border-[#ffffff08] bg-[#1a1a1a] px-3 py-2.5 text-sm text-[#E5E5E5] focus:outline-none focus:border-[#0C8B44]" />
              </label>
            </div>
          ...
          <section className="rounded-2xl border border-[#ffffff08] bg-[#0f1619]/60 p-6 space-y-4">
            ...
            <Toggle label="Two-factor authentication" description="Require a verification code at sign-in" enabled={twoFactor} onChange={(v) => setTwoFactor(v)} />
            <PasskeysPanel />
          ...
          <section className="rounded-2xl border border-[#ffffff08] bg-[#0f1619]/60 p-6 space-y-3">
            ...
            <Link to="/notification-settings" className="flex items-center justify-between rounded-xl border border-[#ffffff08] px-4 py-3 text-sm text-[#E5E5E5] hover:border-[#0C8B44]/40">Notification preferences <span className="text-xs text-[#737373]">Email, push, quiet hours</span></Link>
            <Link to="/linked-wallets" className="flex items-center justify-between rounded-xl border border-[#ffffff08] px-4 py-3 text-sm text-[#E5E5E5] hover:border-[#0C8B44]/40"><span className="inline-flex items-center gap-2"><Wallet className="w-4 h-4 text-[#0C8B44]" /> Linked wallets</span><span className="text-xs text-[#737373]">Web3 addresses</span></Link>
            <Link to="/wallet?action=deposit" className="flex items-center justify-between rounded-xl border border-[#ffffff08] px-4 py-3 text-sm text-[#E5E5E5] hover:border-[#0C8B44]/40"><span className="inline-flex items-center gap-2"><Landmark className="w-4 h-4 text-[#0C8B44]" /> Linked banks</span><span className="text-xs text-[#737373]">ACH destinations</span></Link>
            <Link to="/alerts" className="flex items-center justify-between rounded-xl border border-[#ffffff08] px-4 py-3 text-sm text-[#E5E5E5] hover:border-[#0C8B44]/40"><span className="inline-flex items-center gap-2"><AlertTriangle className="w-4 h-4 text-[#0C8B44]" /> Price alerts</span><span className="text-xs text-[#737373]">Target prices</span></Link>
            <Link to="/referral" className="flex items-center justify-between rounded-xl border border-[#ffffff08] px-4 py-3 text-sm text-[#E5E5E5] hover:border-[#0C8B44]/40"><span className="inline-flex items-center gap-2"><Gift className="w-4 h-4 text-[#0C8B44]" /> Referral program</span><span className="text-xs text-[#737373]">Invite code</span></Link>
            <Link to="/activity" className="flex items-center justify-between rounded-xl border border-[#ffffff08] px-4 py-3 text-sm text-[#E5E5E5] hover:border-[#0C8B44]/40"><span className="inline-flex items-center gap-2"><Activity className="w-4 h-4 text-[#0C8B44]" /> Account activity</span><span className="text-xs text-[#737373]">Login history</span></Link>
            <Link to="/integrations" className="flex items-center justify-between rounded-xl border border-[#ffffff08] px-4 py-3 text-sm text-[#E5E5E5] hover:border-[#0C8B44]/40"><span className="inline-flex items-center gap-2"><Plug className="w-4 h-4 text-[#0C8B44]" /> Integrations</span><span className="text-xs text-[#737373]">Connected apps</span></Link>
          </section>

          <section className="rounded-2xl border border-[#ffffff08] bg-[#0f1619]/60 p-6 space-y-4">
            <h2 className="text-lg font-medium text-[#E5E5E5]">Data & session</h2>
            <div className="flex flex-wrap gap-2">
              <button type="button" onClick={() => void handleExport()} className="inline-flex items-center gap-2 rounded-xl border border-[#ffffff10] px-4 py-2.5 text-sm text-[#E5E5E5] hover:border-[#0C8B44]/40"><Download className="w-4 h-4" /> Export my data</button>
              <button type="button" onClick={handleSignOut} className="inline-flex items-center gap-2 rounded-xl border border-[#ffffff10] px-4 py-2.5 text-sm text-[#E5E5E5] hover:border-[#f44336]/40"><LogOut className="w-4 h-4" /> Sign out</button>
            </div>
            <div className="rounded-xl border border-[#f44336]/20 bg-[#f44336]/5 p-4 space-y-3">
              ...
              <div className="flex gap-2">
                <input value={confirmDelete} onChange={(e) => setConfirmDelete(e.target.value)} placeholder="DELETE" className="flex-1 rounded-xl border border-[#ffffff10] bg-[#070C0E] px-3 py-2 text-sm text-[#E5E5E5] focus:outline-none focus:border-[#f44336]" />
                <button type="button" onClick={() => void handleDelete()} className="rounded-xl bg-[#f44336] px-4 py-2 text-sm text-white">Delete</button>
              </div>
            </div>
          </section>

          <button type="button" onClick={() => void handleSaveProfile()} disabled={saving} className="w-full inline-flex items-center justify-center gap-2 rounded-xl bg-[#0C8B44] py-3.5 text-sm font-medium text-white hover:bg-[#0a7539] disabled:opacity-50">
