...
import { ArrowLeft, Shield, Upload, CheckCircle, Clock, AlertCircle, User, Camera, Zap } from 'lucide-react'
...
function KYCInner() {
  ...
  useEffect(() => {
    api.getKycStatus()
      .then(s => {
        ...
        setTierInfo({ tier: s.tier, tierLimits: s.tierLimits })
      })
  ...
  const tierColor = tierInfo ? TIER_COLORS[tierInfo.tier] : '#737373'
    ...
    return (
      <div className="min-h-screen bg-[#070C0E] flex items-center justify-center">
        ...
        <div className="text-center pt-24 max-w-md mx-auto px-6">
          ...
          {tierInfo && (
            <div className="mb-4 inline-block px-3 py-1 rounded-full text-xs font-semibold" style={{ background: `${tierColor}20`, color: tierColor }}>
              {tierInfo.tier}
            </div>
          )}
          <p className="text-sm text-[#737373] mb-6">
            {kycStatus === 'approved' && tierInfo?.tierLimits && (
              <>
                Your identity has been verified. Tier {tierInfo.tier} limits apply.
                <br />
                Daily Withdraw: ${tierInfo.tierLimits.dailyWithdrawLimit.toLocaleString()}
              </>
            )}
    ...
    return (
      <div className="min-h-screen bg-[#070C0E] flex items-center justify-center">
        ...
        <div className="text-center pt-24 max-w-md mx-auto px-6">
          ...
          <div className="mb-4 inline-flex items-center gap-2 px-3 py-1.5 rounded-lg bg-[#0C8B44]/10 border border-[#0C8B44]/20">
            <Zap className="w-4 h-4 text-[#0C8B44]" />
            <span className="text-xs text-[#0C8B44] font-medium">You'll reach Tier 2 once approved</span>
          </div>
          ...
          <div className="flex items-center justify-between mb-8">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-[#737373]">Tier 2: $50k daily withdrawal limit</p>
              </div>
            </div>
            {tierInfo && (
              <div className="text-right">
                <p className="text-xs text-[#737373] uppercase tracking-wider">Your Tier</p>
                <p className="text-lg font-bold" style={{ color: tierColor }}>{tierInfo.tier}</p>
              </div>
            )}
          </div>
                ...
                <div>
                  ...
                  {idFile ? (
                    ...
                    <button onClick={() => fakeUpload(setIdFile)} className="w-full py-6 border-2 border-dashed border-[#ffffff15] rounded-lg hover:border-[#0C8B44]/50 transition-colors flex flex-col items-center gap-2">
                      ...
                      <span className="text-[10px] text-[#737373]">JPG, PNG or PDF · Max 8 MB</span>
                    </button>
                  )}
                </div>
              ...
              <div className="space-y-4">
                ...
                {selfieFile ? (
                  ...
                  <button onClick={() => fakeUpload(setSelfieFile)} className="w-full py-10 border-2 border-dashed border-[#ffffff15] rounded-xl hover:border-[#0C8B44]/50 transition-colors flex flex-col items-center gap-3">
                    ...
                    <span className="text-[10px] text-[#737373]">JPG or PNG · Max 8 MB</span>
                  </button>
                )}
                <div className="flex gap-3">
            ...
            {step === 'review' && (
              <div className="space-y-4">
                ...
                <div className="flex items-start gap-2 rounded-xl bg-[#0a0f11] border border-[#ffffff08] p-3 mt-4">
                  ...
                  <p className="text-[11px] text-[#737373]">By submitting you confirm that all information is accurate and agree to our identity verification terms. Review typically takes 1–2 business days. You'll reach Tier 2 once approved.</p>
                </div>
                <div className="flex gap-3">
