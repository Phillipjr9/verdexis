...
export const adminApi = {
  ...
  listPendingReviews: () =>
    request<{ reviews: Array<{ id: string; rating: number; text: string; authorName: string; authorAvatar: string | null; approved: boolean; createdAt: string; updatedAt: string; user: { id: string; email: string; name: string } }> }>('/api/admin/reviews?status=pending'),
  ...
  listOnchainDeposits: (status: 'pending' | 'credited' | 'rejected' | 'all' = 'pending') =>
    request<{ pendingDeposits: Array<{ id: string; userId: string; txHash: string; chainId: string; toAddress: string; fromAddress: string; asset: string; amount: number; status: string; note: string | null; creditedTxId: string | null; createdAt: string; explorerUrl: string; user: { id: string; email: string; name: string; investmentId: string | null } }> }>(`/api/admin/pending-deposits?status=${status}`),
  ...
  deposit: (userId: string, input: { currency: string; symbol?: string; amount: number; reason: string; note?: string; status?: 'pending' | 'completed'; notify?: boolean; occurredAt?: string; investAs?: { symbol: string; name: string; type: 'crypto' | 'stock' | 'etf' } }) =>
    request<{ balance?: AdminWalletBalance; holding?: { id: string; symbol: string; name: string; amount: number; avgPrice: number; type: string }; transaction: AdminTransaction; historicalPrice?: number; quantity?: number; currentPrice?: number | null; unrealizedPnl?: number | null; unrealizedPnlPct?: number | null }>(`/api/admin/users/${userId}/deposit`, { method: 'POST', body: JSON.stringify(input) }),
  ...
  upsertUserWalletLink: (userId: string, input: { address: string; chainId?: string; provider?: string; label?: string; setPrimary?: boolean }) =>
  ...
  listPendingWithdrawals: () =>
    request<{ withdrawals: Array<{ id: string; userId: string; amount: number; asset: string; status: string; createdAt: string; walletLink: { address: string; chainId: string | null } | null; user: { id: string; email: string; name: string } }> }>('/api/withdrawals/admin/pending'),
  approveWithdrawal: (id: string, txHash: string) =>
    request<{ withdrawal: { id: string; status: string } }>(`/api/withdrawals/admin/${id}/approve`, { method: 'PUT', body: JSON.stringify({ txHash }) }),
  rejectWithdrawal: (id: string, reason: string) =>
    request<{ withdrawal: { id: string; status: string } }>(`/api/withdrawals/admin/${id}/reject`, { method: 'PUT', body: JSON.stringify({ reason }) }),
  ...
  // Optional fee-payment proofs submitted by users
  listUserFeeProofs: (userId: string, email?: string) => {
    const qs = email ? `?email=${encodeURIComponent(email)}` : ''
    return request<{ proofs: Array<Record<string, unknown>> }>(`/api/fee-proofs/admin/user/${userId}${qs}`)
  },
  listPendingFeeProofs: () =>
    request<{ proofs: Array<Record<string, unknown>> }>('/api/fee-proofs/admin/pending'),
  updateFeeProof: (id: string, input: { status: 'pending' | 'verified' | 'rejected'; reviewerNote?: string }) =>
    request<{ proof: Record<string, unknown> }>(`/api/fee-proofs/admin/${id}`, {
      method: 'PATCH',
      body: JSON.stringify(input),
    }),
}
