...
export const api = {
  ...
  verifyOtp: (pendingToken: string, code: string) =>
    request('/api/auth/verify-otp', { method: 'POST', body: JSON.stringify({ pendingToken, code }) }),
  ...
  loginResendOtp: (pendingToken: string) =>
    request('/api/auth/login-resend-otp', { method: 'POST', body: JSON.stringify({ pendingToken }) }),
