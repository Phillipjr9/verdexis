...
function AuthModal({ isOpen, onClose, defaultMode = 'login' }: AuthModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (mode === 'forgot') {
      ...
      try {
        ...
        toast.success('Reset link sent', {
          description: `If that email exists, a reset link has been sent to ${safeEmail}.`,
        })
      } catch (err) {
    ...
    try {
      if (mode === 'otp') {
        ...
        toast.success(
          pendingFlow === 'signup' ? 'Email verified and account created' : 'Welcome back'
        )
        setLoading(false)
        onClose()
      ...
      if ('otpRequired' in result || 'pendingToken' in result) {
        const r = result as { pendingToken: string; message?: string }
  ...
  return createPortal(
    <div className="fixed inset-0 z-[100] flex items-start sm:items-center justify-center p-4 overflow-y-auto">
      ...
      <div
        className="relative w-full max-w-md glass-card overflow-hidden my-auto"
        style={{ background: 'rgba(15,22,25,0.95)', backdropFilter: 'blur(24px)' }}
      >
        <button
          type="button"
          aria-label="Close"
        ...
        <div className="p-5 sm:p-8">
            ...
            {mode === 'otp' && (
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-3 bg-[#1a1a1a] border border-[#ffffff08] rounded-xl text-sm text-[#E5E5E5] text-center tracking-[0.4em] text-lg"
                  placeholder="000000"
                  autoFocus
                />
                {error && (
                ...
                <button
                  ...
                  className="w-full flex items-center justify-center gap-2 py-3.5 bg-[#0C8B44] text-white text-sm font-medium rounded-xl mt-4 disabled:opacity-50"
                >
                  {loading ? (
                ...
                <button
                  ...
                  className="w-full mt-2 text-xs text-[#737373]"
                >
                  ← Back to sign in
                </button>
              </div>
            ...
            {mode !== 'otp' && (
              <>
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="w-full pl-10 pr-4 py-3 bg-[#1a1a1a] border border-[#ffffff08] rounded-xl text-sm text-[#E5E5E5]"
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        className="w-full pl-10 pr-12 py-3 bg-[#1a1a1a] border border-[#ffffff08] rounded-xl text-sm text-[#E5E5E5]"
                    ...
                    {mode === 'login' && (
                      <button
                        type="button"
                        onClick={goForgot}
                        className="mt-2 text-xs text-[#737373] hover:text-[#0C8B44]"
                      >
                        Forgot password?
                      </button>
                    )}
                ...
                <button
                  type="submit"
                  disabled={loading}
                  className="w-full flex items-center justify-center gap-2 py-3.5 bg-[#0C8B44] text-white text-sm font-medium rounded-xl disabled:opacity-50"
                >
                  {loading ? (
                    <div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin" />
                  ) : (
                ...
                {mode === 'login' && (
                  <button
                    ...
                    className="w-full flex items-center justify-center gap-2 py-3.5 bg-[#1a1a1a] border border-[#ffffff15] text-[#E5E5E5] text-sm font-medium rounded-xl"
                  >
          ...
          {mode !== 'otp' && (
            <p className="text-center text-sm text-[#737373] mt-6">
              {mode === 'forgot' ? (
                <button onClick={goBackToLogin} className="text-[#0C8B44] font-medium">
                  Back to sign in
                </button>
              ) : mode === 'login' ? (
                <>
