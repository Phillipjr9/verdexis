...
// Curated seed testimonials shown alongside real submissions.
...
export default function Testimonials({ onSignInRequired }: { onSignInRequired?: () => void }) {
  ...
  const load = () => {
    if (typeof api.listReviews !== 'function') {
      setLoaded(true)
      return
    }
    api
  ...
  useEffect(() => {
    load()
    if (getToken() && typeof api.getMyReview === 'function') {
      api
        .getMyReview()
        .then((r) => {
          if (r.review) {
  ...
  const submit = async () => {
    ...
    if (trimmed.length > 400) {
      toast.error('Please keep it under 400 characters')
      return
    }
    ...
    if (typeof api.upsertReview !== 'function') {
      toast.error('Reviews are temporarily unavailable')
      return
    }
    setSubmitting(true)
    try {
      ...
      toast.success(
        hasMine
          ? 'Review updated and is pending admin approval.'
          ...
          <ul className="flex gap-4 w-max">
            {!loaded &&
              Array.from({ length: 4 }).map((_, i) => (
                <li
                  key={`skel-${i}`}
                  className="snap-start shrink-0 w-[280px] sm:w-[320px] h-48 rounded-2xl bg-[#0f1619]/60 border border-[#ffffff05] animate-pulse"
                />
              ))}
            {loaded &&
              display.map((t) => (
                ...
                >
                  ...
                  <div className="flex items-center gap-0.5 mb-2" aria-label={`${t.rating} out of 5 stars`}>
                    {Array.from({ length: 5 }, (_, i) => (
                      <Star
                        key={`test-star-${i}`}
                        className={`w-3.5 h-3.5 ${i < t.rating ? 'text-[#F57C00]' : 'text-[#333]'}`}
                        fill={i < t.rating ? '#F57C00' : 'none'}
                      />
                    ))}
                  </div>
                  ...
                  <div className="flex items-center gap-2.5 pt-4 mt-4 border-t border-[#ffffff08]">
                    {t.authorAvatar ? (
                      <img
                        src={t.authorAvatar}
                        alt=""
                        className="w-9 h-9 rounded-full object-cover ring-1 ring-[#ffffff10]"
                      />
                    ) : (
                      <div
                    ...
                    {!t.id.startsWith('seed:') && (
                      <CheckCircle2
                        className="ml-auto w-3.5 h-3.5 text-[#0C8B44]"
          ...
          <div className="w-full max-w-md rounded-2xl bg-[#0f1619] border border-[#ffffff10] p-6 shadow-2xl">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-medium text-[#E5E5E5]">
                {hasMine ? 'Edit your review' : 'Leave a review'}
              </h3>
              <button
                onClick={() => setFormOpen(false)}
                className="p-1 rounded hover:bg-[#ffffff08] text-[#737373]"
                aria-label="Close"
              >
                <X className="w-4 h-4" />
              </button>
            ...
            <p className="text-xs text-[#737373] mb-4">
              Your name and avatar from your profile will be shown next to this review on the homepage.
            </p>
            ...
            <div className="mb-4">
              ...
              <div className="flex items-center gap-1 mt-1.5" role="radiogroup" aria-label="Rating">
                {[1, 2, 3, 4, 5].map((n) => (
                  ...
                  >
                    <Star
                      className={`w-7 h-7 ${n <= rating ? 'text-[#F57C00]' : 'text-[#333]'}`}
                      fill={n <= rating ? '#F57C00' : 'none'}
                    />
                  </button>
                ))}
              </div>
            ...
            <div className="mb-4">
              <label htmlFor="review-text" className="text-[10px] uppercase tracking-[0.05em] text-[#737373]">
                Your review
              </label>
              <textarea
                id="review-text"
              ...
              {hasMine && (
                <button
                  ...
                  onClick={async () => {
                    ...
                    try {
                      if (typeof api.deleteMyReview === 'function') await api.deleteMyReview()
